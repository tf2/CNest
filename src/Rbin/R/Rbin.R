...
generate_correlation <- function(bin_dir, cor_dir, sample_name, index_file) {
	...
	filenames = paste0(bin_dir, "/", filenames)
	rr1= .C( "getValues",
				"filename" = as.character(sample_name),
		 		"start_position" = as.double(0), 
		  		"number_row_to_read" =as.integer(nrow(index)),
		   		"values" = as.double(1:nrow(index)),
		   		"PACKAGE" = "Rbin"
...
get_references <- function(sample_name, index_file = "index.txt", gender_file, outputdir = "../logr", correlation_path="../cors/", log2_path = "../txt_covered/",	batch_size = 1000) {
