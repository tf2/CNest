...
`RbinCompress_rbin` <- function(binname, filenames, start_pos, n_rows_to_read, values) {
	 .C("compress_rbin", "binname"=as.character(binname), "filenames"=as.character(filenames), "n"=as.integer(length(filenames)), "starting_pos"=as.integer(start_pos), "number_row_to_read"=as.integer(n_rows_to_read),"PACKAGE" = "Rbin")
}
...
`compress_rbins_into_chr_cbins` <- function(samplefile, cnfile_dir, cnfile_name, chr, rbin_dir, index_file) {
	...
	offsets <- .C("geteOffsets_ratio"
					, "filename"=as.character(index_file)
					, "chr"=as.integer(chr)
					, "start"=as.integer(start)
					, "stop"=as.integer(stop)
					, "rows"=as.integer(0)
...
`write_blank_cbin` <- function(binname, n_samples, n_rows) {
	.C("write_blank_cbin_file", "binname"=as.character(binname), "n"=as.integer(n_samples), "number_row_to_read"=as.integer(n_rows), "PACKAGE" = "Rbin")
}

`write_sample_to_blank_cbin` <- function(binname, filename, sample_index, number_samples, starting_pos, number_row_to_read) {
 .C("write_sample_to_blank_cbin"
...
`compress_rbins_into_chr_cbins_chunk` <- function(samplefile, cnfile_dir, cnfile_name, chr, rbin_dir, index_file, chunk_size = 200) {
	...
	c = .C("exome_ratio_read_write_chunk_array"
			...
			, "starting_pos"=as.integer(offsets$offset)
			, "PACKAGE" = "Rbin")
